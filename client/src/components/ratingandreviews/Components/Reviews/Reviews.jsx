...
function Reviews(props) {
  ...
  const searchReviews = (reviewList, input) => {
    ...
    return reviewList.filter((review) => {
      ...
      return (
        summary.includes(input.toLowerCase()) ||
        body.includes(input.toLowerCase())
      );
    });
  };
        ...
        <>
          <Card>
            <Stack
              ...
              gap={2}
              className='reviews-dropdown-stack'
            >
              <div>Displaying</div>
              <b>
                {searchResults.length} / {props.reviewList.length}
              </b>
            ...
            <Card.Body>
              <Stack gap={3}>
                {searchResults.map((review) => (
                  <ReviewTile key={review.reviews_id} review={review} />
                ))}
                {props.reviewList.length > 2 &&
                reviewNum < props.reviewList.length ? (
                  <Button
                    className='show-more'
