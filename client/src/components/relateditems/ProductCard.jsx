...
class ProductCard extends React.Component {
  ...
  componentDidMount() {
    ...
    if (window.localStorage[`${this.props.product.id}/styles`]) {
      let currentStyle = JSON.parse(
        window.localStorage[`${this.props.product.id}/styles`]
      );
      this.setState({
      ...
      axios
        .get(
          `http://13.57.182.185/api/products/${this.props.product.id}/styles`
        )
        .then((product) => {
          if (this._isMounted) {
    ...
    if (window.localStorage[`meta/?product_id=${this.props.product.id}`]) {
      let metaData = JSON.parse(
        window.localStorage[`meta/?product_id=${this.props.product.id}`]
      );
      ...
      let rating =
        (star1 * 1 + star2 * 2 + star3 * 3 + star4 * 4 + star5 * 5) / total;
      this.setState({
        rating: rating,
      });
    } else {
      axios
        .get(
          `http://54.163.77.29/api/product/${this.props.product.id}/reviews/meta`
        )
        .then((meta) => {
          window.localStorage[`meta/?product_id=${this.props.product.id}`] =
    ...
    return (
      ...
      >
        ...
        <ActionButton
          ...
          featuredProd={this.props.featuredProd}
        />
